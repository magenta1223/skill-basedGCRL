...
class GoalConditioned_GoalPrompt_Prior(BaseModule):
    ...
    def __train__(self, inputs):
        ...
        # -------------- Subgoal Generator -------------- #
        if self.training:
            goal = self.goal_prompt + (G - self.goal_prompt.unsqueeze(0)).detach()
        else:
            goal = self.goal_prompt.unsqueeze(0).repeat(G.shape[0], 1)
        sg_input = torch.cat((start,  goal), dim = -1)
    ...
    def __eval__(self, inputs):
        ...
        if "G" in inputs.keys():
            ...
            state, G = inputs['states'], self.goal_prompt.unsqueeze(0)   
    ...
    def __finetune__(self, inputs):
        ...
        if "G" in inputs.keys():
            ...
            states, G, next_states = inputs['states'], self.goal_prompt.unsqueeze(0) , inputs['next_states'] 
