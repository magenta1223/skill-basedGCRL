...
class GCRL_Skill_Trainer:
    ...
    def train_policy(self, n_ep, seed):
        ...
        for task in self.tasks:
            with self.collector.env.set_task(task), self.agent.policy.expl(), self.collector.low_actor.expl(): #, collector.env.step_render():
            ...
            log[f'{str(task)}/return'] = sum(episode.rewards)
            ...
            avg_rwd.append(sum(episode.rewards))

            # ------------- Visualize ------------- #
            if (n_ep + 1) % self.cfg.render_period == 0:
                log[f'{str(task)}/policy_vis'] = self.visualize(task)

        print("\n# ------------------------------------------ #\n")
        log['avg_rwd'] = np.mean(avg_rwd)
        # ------------- Precollect phase ------------- #
    ...
    def n_step(self, episode):
        return int(len(self.tasks) * self.cfg.reuse_rate * len(episode) / self.cfg.rl_batch_size)
